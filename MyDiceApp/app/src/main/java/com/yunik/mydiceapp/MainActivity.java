...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button button = findViewById(R.id.btnRollTheDice);

        final MediaPlayer mp = MediaPlayer.create(this, R.raw.dice_sound);
        button.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Log.i("MyDiceApp","btn roll tapped");

                Random rand = new Random();

                int myRandomNumber = rand.nextInt(6);

                Log.i("MyDiceApp", myRandomNumber+"");
                System.out.println("ok");

                diceImage1.setImageResource(diceImages[myRandomNumber]);
                myRandomNumber = rand.nextInt(6);
